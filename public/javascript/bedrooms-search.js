async function bedroomsHandler(event) {
    ...
    if (response.ok) {
        ...
        if (event.target.value == "1") {
            const filteredData = data.filter(index => (index.beds == 1))
            renderHtml(filteredData)
        } else if (event.target.value == "2") {
            const filteredData = data.filter(index => (index.beds == 2))
            renderHtml(filteredData)
        } else if (event.target.value == "3") {
            const filteredData = data.filter(index => (index.beds == 3))
            renderHtml(filteredData)
        } else if (event.target.value == "4+") {
            const filteredData = data.filter(index => (index.beds >= 4))
            renderHtml(filteredData)
...
document.querySelector('.bedrooms-search').addEventListener('change', bedroomsHandler);